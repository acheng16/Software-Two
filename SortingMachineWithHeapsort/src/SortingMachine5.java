...
public class SortingMachine5<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
         ...
         */
        Array<T> answer = new Array1L<T>(q.length());
        for (int i = 0; q.length() != 0; i++) {
            T removed = q.dequeue();
            answer.setEntry(i, removed);
        }

        heapify(answer, 0, order);
    ...
    public final T removeFirst() {
        ...
        if (this.heap.length() > 1) {
            this.heap.exchangeEntries(0, this.heap.length() - 1);
        }
